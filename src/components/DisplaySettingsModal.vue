...
watch(aiChatEnabled, (enabled) => {
    if (!enabled && section.value === 'models') {
        selectSection('display')
    }
})

function selectSection(next: SettingsSection) {
    if (next === 'models' && !aiChatEnabled.value) return
    section.value = next
...
/** Triple-click About logo opens DevTools when remote `devtools` is on. */
const LOGO_CLICK_WINDOW_MS = 1200
let logoClickCount = 0
let logoClickTimer: ReturnType<typeof setTimeout> | null = null

function onAboutLogoClick() {
    if (!note.devtoolsEnabled) return
    logoClickCount += 1
    if (logoClickTimer) clearTimeout(logoClickTimer)
    if (logoClickCount >= 3) {
        logoClickCount = 0
        void note.tryOpenDevtools()
        return
    }
            ...
            <div class="settings-layout">
                ...
                <div class="settings-pane">
                    <div v-if="section === 'display'" class="pane-scroll">
                        ...
                        <div class="accent-row">
                            ...
                            >
                                ...
                                <span class="swatch-label">{{
                                    t('displaySettings.accentDefault')
                                }}</span>
                            </button>
                            <button
                                ...
                                class="accent-swatch"
                                :class="{
                                    active: displayPrefs.accentColor === hex,
                                }"
                                :style="{ '--swatch': hex }"
                                :title="hex"
                                :aria-label="hex"
                        ...
                        <div class="section-label">
                            {{ t('displaySettings.sidebarSection') }}
                        </div>
                        <div class="check-grid">
                            <label class="check">
                                <input
                                    v-model="displayPrefs.sidebar.system"
                                    type="checkbox"
                                />
                                <span>{{
                                    t('displaySettings.sidebarSystem')
                                }}</span>
                            </label>
                            <label class="check">
                                <input
                                    v-model="displayPrefs.sidebar.resources"
                                    type="checkbox"
                                />
                                <span>{{
                                    t('displaySettings.sidebarResources')
                                }}</span>
                            </label>
                            <label class="check">
                                <input
                                    v-model="displayPrefs.sidebar.processes"
                                    type="checkbox"
                                />
                                <span>{{
                                    t('displaySettings.sidebarProcesses')
                                }}</span>
                            </label>
                            <label class="check">
                                <input
                                    v-model="displayPrefs.sidebar.network"
                                    type="checkbox"
                                />
                                <span>{{
                                    t('displaySettings.sidebarNetwork')
                                }}</span>
                            </label>
                        </div>

                        <div class="section-label">
                            {{ t('displaySettings.coreSection') }}
                        </div>
                        <div class="check-grid">
                            <label class="check">
                                <input
                                    v-model="displayPrefs.explorer.show"
                                    type="checkbox"
                                />
                        ...
                        <div class="section-hint">
                            {{ t('displaySettings.attrColumns') }}
                        </div>
                        <div class="check-grid nested">
                            <label class="check">
                                <input
                                    v-model="displayPrefs.explorer.colName"
                                    type="checkbox"
                                />
                            ...
                            <label class="check">
                                <input
                                    v-model="displayPrefs.explorer.colSize"
                                    type="checkbox"
                                />
                            ...
                            <label class="check">
                                <input
                                    v-model="displayPrefs.explorer.colType"
                                    type="checkbox"
                                />
                            ...
                            <label class="check">
                                <input
                                    v-model="displayPrefs.explorer.colModified"
                                    type="checkbox"
                                />
                            ...
                            <label class="check">
                                <input
                                    v-model="
                                        displayPrefs.explorer.colPermissions
                                    "
                                    type="checkbox"
                                />
                            ...
                            <label class="check">
                                <input
                                    v-model="displayPrefs.explorer.colGroup"
                                    type="checkbox"
                                />
                        ...
                        <template v-if="aiChatEnabled">
                            <div class="section-label">
                                {{ t('displaySettings.aiSection') }}
                            </div>
                            <div class="check-grid">
                                <label class="check">
                                    <input
                                        v-model="displayPrefs.aiPanel"
                                        type="checkbox"
                                    />
                                    <span>{{
                                        t('displaySettings.aiPanel')
                                    }}</span>
                                </label>
                            </div>
                        </template>
                    ...
                    <div
                        v-else-if="section === 'models' && aiChatEnabled"
                        class="pane-scroll models-pane"
                    >
                        <AiSettingsPane />
                    </div>
