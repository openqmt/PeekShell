...
function readStoredEntriesWidth() {
    ...
    return Math.min(
        MAX_ENTRIES_WIDTH,
        Math.max(MIN_ENTRIES_WIDTH, Math.round(raw)),
    )
}
...
const previewLimitBytes = computed(() =>
    previewMaxBytes(explorerPrefsState.value),
)
...
const attrGridStyle = computed(() => ({
    gridTemplateColumns:
        visibleAttrCols.value.map((key) => ATTR_COL_WIDTHS[key]).join(' ') ||
        '1fr',
}))
...
function resolveDropRemoteDir(position?: {
    x: number
    y: number
}): string | null {
...
function formatType(entry: Pick<RemoteEntry, 'isDir' | 'fileType'>) {
    ...
    if (entry.isDir || raw.includes('directory') || raw === 'd')
        return t('explorer.typeDir')
    if (raw.includes('link') || raw === 'l' || raw === 'symlink')
        return t('explorer.typeSymlink')
    if (raw === 'f' || raw === 'file' || raw.includes('regular'))
        return t('explorer.typeFile')
...
function clampHeight(value: number) {
    ...
    return Math.min(
        ...
        Math.max(MIN_HEIGHT, Math.round(value)),
    )
}
...
async function ctxEditorCopy() {
    if (ctxMenu.value?.variant !== 'editor' || !ctxMenu.value.hasSelection)
        return
    closeCtxMenu()
...
async function downloadEntry(entry: RemoteEntry) {
    closeCtxMenu()
    if (!activeSessionId.value) return
    const defaultName = entry.isDir
        ? `${basenameOf(entry.path)}.tar.gz`
    ...
    try {
        for (const id of ids) {
            ...
            try {
                ...
                if (
                    transfers.isAborted(abortAt) ||
                    String(e).includes('已停止')
                )
                    break
...
<template>
    ...
    >
        ...
        <div class="toolbar">
            ...
            <div class="transfers-wrap">
                <button
                    ...
                    :title="
                        hasBatchProgress
                            ? t('transfers.batchProgress', {
                                  done: transfers.batchCompleted,
                                  total: transfers.batchTotal,
                              })
                            : t('transfers.title')
                    "
                ...
                >
                    ...
                    <span
                        v-if="batchProgressLabel"
                        class="transfers-badge wide"
                        >{{ batchProgressLabel }}</span
                    >
                    <span v-else-if="pendingCount" class="transfers-badge">{{
                        pendingCount
                    }}</span>
                </button>

                <div
                    v-if="transfersPanelOpen"
                ...
                >
                    <div class="transfers-head">
                        <strong>
                            {{ t('transfers.title') }}
                            <span
                                v-if="hasBatchProgress"
                                class="transfers-batch"
                            >
                                {{
                                    t('transfers.batchProgress', {
                                        done: transfers.batchCompleted,
                        ...
                        <div class="transfers-head-actions">
                            ...
                            <button
                                ...
                                :title="
                                    resuming
                                        ? t('transfers.resuming')
                                        : t('transfers.continue')
                                "
                                :aria-label="
                                    resuming
                                        ? t('transfers.resuming')
                    ...
                    <div class="transfers-dir">
                        <div class="transfers-dir-label">
                            {{ t('transfers.defaultDir') }}
                        </div>
                        <div class="transfers-dir-row">
                            <span
                                class="transfers-dir-path"
                                :title="
                                    defaultDownloadDir ||
                                    t('transfers.defaultDirHint')
                                "
                            >
                                {{
                                    defaultDownloadDir ||
                    ...
                    <div v-if="!transferItems.length" class="transfers-empty">
                        {{ t('transfers.empty') }}
                    </div>
                            ...
                            <div class="transfer-meta">
                                <span>
                                    ...
                                    <template v-if="item.total">
                                        /
                                        {{ formatBytes(item.total) }}</template
                                    >
                                </span>
                                <span v-if="item.total"
                                    >{{ transferPercent(item) }}%</span
                                >
                            </div>
        ...
        <div
            class="panes"
            :style="{ gridTemplateColumns: `${entriesWidth}px 1fr` }"
        >
            <div ref="entriesEl" class="entries">
                <div v-if="!activeSessionId" class="placeholder">
                    {{ t('explorer.needConnect') }}
                </div>
                <div v-else-if="treeLoading" class="placeholder">
                    {{ t('common.loading') }}
                </div>
                <div v-else-if="!treeRows.length" class="placeholder">
                    {{ t('explorer.emptyDir') }}
                </div>
                <button
                ...
                >
                    ...
                    <span v-else class="kind">{{
                        row.entry.isDir ? 'DIR' : 'FILE'
                    }}</span>
            ...
            <div
                class="preview"
                @contextmenu.prevent="onPreviewBlankContextMenu"
            >
                <div v-if="!activeSessionId" class="placeholder">
                ...
                <div v-else-if="!selectedPath" class="placeholder">
                    {{ t('explorer.selectItem') }}
                </div>
                ...
                <template v-else-if="selectedIsDir">
                    <div
                        ...
                        :style="attrGridStyle"
                    >
                        <span v-for="col in visibleAttrCols" :key="col">{{
                            t(`explorer.${col}`)
                        }}</span>
                    </div>
                    <div v-if="!folderEntries.length" class="placeholder">
                        {{ t('explorer.emptyDir') }}
                    </div>
                    <button
                ...
                <template v-else-if="preview">
                    <div
                        ...
                        :style="attrGridStyle"
                        @contextmenu.prevent.stop="
                            onEntryContextMenu(previewAsEntry(preview), $event)
                        "
                    >
                        <span
                            v-for="col in visibleAttrCols"
                    ...
                    <div
                        ...
                        "
                    >
                        {{ t('explorer.truncated', { n: previewLimitKb }) }} —
                        {{ t('explorer.truncatedReadonly') }}
                    </div>
            ...
            >
                ...
                <template v-else>
                    ...
                    <button
                        ...
                        class="ctx-item"
                        :disabled="actionBusy"
                        @click="downloadEntry(ctxMenu.entry)"
                    >
                        {{ t('explorer.download') }}
                    </button>
                    <button
                    ...
                    <button
                        ...
                        class="ctx-item"
                        :disabled="actionBusy"
                        @click="openPrompt('rename', ctxMenu.entry)"
                    >
                        {{ t('explorer.rename') }}
                    </button>
                    <button
                        ...
                        class="ctx-item"
                        :disabled="actionBusy"
                        @click="openPrompt('chmod', ctxMenu.entry)"
                    >
        ...
        >
            <div class="prompt-box" role="dialog" :aria-label="promptTitle">
                <h3>{{ promptTitle }}</h3>
                <p v-if="promptKind === 'delete'" class="prompt-message">
                    {{ promptLabel }}
                </p>
                ...
                <div class="prompt-actions">
                    ...
                    <button
                        ...
                        :disabled="
                            actionBusy ||
                            (promptKind !== 'delete' && !promptInput.trim())
                        "
                        @click="submitPrompt"
                    >
                        {{
