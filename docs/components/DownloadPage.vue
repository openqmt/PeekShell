...
const COPY = {
  ...
  en: {
    ...
    noAssets: 'No desktop release assets are available in the latest release.',
...
const recommendation = computed<Recommendation | null>(() => {
  ...
  if (system.platform && system.arch) {
    const exact = pickPreferredAsset(
      assets.value.filter(
        (asset) =>
          asset.platform === system.platform && asset.arch === system.arch,
      ),
    )
      ...
      <section class="dl-browse" aria-label="platforms">
        ...
        <div class="dl-tabs" role="tablist">
          ...
          >
            ...
            <em v-if="detectedSystem.platform === platform">{{ ui.detected }}</em>
          ...
          >
            ...
            <ul class="dl-list">
              <li v-for="asset in group.assets" :key="asset.id">
                <a
                  class="dl-row"
                  :class="{ recommended: isRecommended(asset) }"
                  :href="asset.url"
                  target="_blank"
                  rel="noreferrer"
                >
                  <div class="dl-row-main">
                    <strong>
                      ...
                      <span v-if="isRecommended(asset)" class="dl-row-badge">
                        {{ ui.recommended }}
                      </span>
                    </strong>
                  ...
                  <div class="dl-row-meta">
                    ...
                    <span class="dl-row-cta">{{ ui.directDownload }}</span>
                  </div>
                </a>
              </li>
            </ul>
...
.dl-recommend {
  ...
  border-radius: 20px;
  background:
    radial-gradient(circle at top right, var(--vp-c-brand-soft), transparent 42%),
    var(--vp-c-bg-soft);
}
...
.dl-tab.active {
  background: var(--vp-c-brand-1);
  color: #fff;
  box-shadow: 0 2px 8px color-mix(in srgb, var(--vp-c-brand-1) 35%, transparent);
}
